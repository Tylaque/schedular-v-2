...
export default function CertificationsClient({
  ...
  async function handleCreate() {
    ...
    setMsg(null);
    try {
      const result = await createCertificationAction(createName, createDesc);
      if (result.ok) {
        setItems((prev) => [...prev, result.certification]);
        setCreateName("");
        setCreateDesc("");
        setMsg({ type: "ok", text: "Certification added." });
      } else {
  ...
  function startEdit(item: CertificationRecord) {
  ...
  async function handleUpdate() {
    ...
    setMsg(null);
    try {
      const result = await updateCertificationAction(editingId, editName, editDesc);
      if (result.ok) {
        setItems((prev) => prev.map((c) => (c.id === editingId ? result.certification : c)));
        setEditingId(null);
        setMsg({ type: "ok", text: "Certification updated." });
      } else {
        setMsg({ type: "err", text: result.reason });
      }
    } catch (err) {
      showError(err, "Failed to update certification.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Delete this certification? It will be removed from every associate and every project that references it.")) {
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      ...
      if (result.ok) {
        ...
        setMsg({ type: "ok", text: "Certification deleted." });
      } else {
        setMsg({ type: "err", text: result.reason });
      }
    } catch (err) {
      showError(err, "Failed to delete certification.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 mb-6 dark:border-gray-700 dark:bg-gray-900">
        <h2 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2 dark:text-gray-50">
          <Plus className="w-4 h-4 text-gray-400" /> Add a certification
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <input
            ...
            placeholder="Certification name (e.g. ISO 27001 Auditor)"
